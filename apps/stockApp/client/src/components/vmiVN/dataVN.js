...
const getSymbolListVN = async (symbol, key) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/search?keywords=${symbol}&type=symbol&offset=0&limit=20`, {
            headers: {
...
const getCompanyProfileVN = async (symbol, key) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/symbols/${symbol}/fundamental`, {
            headers: {
                Authorization: `Bearer ${key}`,
                Host: "restv2.fireant.vn"
            }
...
const getIncomeStatementVN = async (symbol, key) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/symbols/${symbol}/full-financial-reports?type=2&year=2022&quarter=0&limit=6`, {
            headers: {
                Authorization: `Bearer ${key}`,
                Host: "restv2.fireant.vn"
            }
...
const getBalanceSheetVN = async (symbol, key = process.env.REACT_APP_FIREANT_KEY) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/symbols/${symbol}/full-financial-reports?type=1&year=2022&quarter=3&limit=6`, {
            headers: {
...
const getCashFLowVN = async (symbol = "FPT", key = process.env.REACT_APP_FIREANT_KEY) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/symbols/${symbol}/full-financial-reports?type=4&year=2022&quarter=0&limit=6`, {
            headers: {
                Authorization: "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsIng1dCI6IkdYdExONzViZlZQakdvNERWdjV4QkRITHpnSSIsImtpZCI6IkdYdExONzViZlZQakdvNERWdjV4QkRITHpnSSJ9..pZEgQ4RH7TOUSWo_eezAFumnxZzVMfzsM_nG1IKeFqs396uAeJuRpAgQl481S-MA-S2nqcq0yoUlUwaq4zBGs7qCFWuWMafuzRbyGt-IKHoY4QplILe1y7nODEgeOQXC8FY-qmtYK_Nbs5oEvYq_K-QUX3dqjlUwi_sPJwa4mPQd9Dow7-3IenniVAuqA9XW1XY2XEp6TkxcmZ-5cbnexTOECiIj_-XdYul0QVOFCZlEt2iEmFbRgdpPGP96e-YbmKgWswoCFXij81nsiHxDmm4Hu_AHi2-ZR9KM4XIe-eeKr2q-pMzkS2Ii9iVcEqHnvSq5eYb46W_qPB4ZmK-3pQ",
                Host: "restv2.fireant.vn"
            }
...
const getRatioVN = async (symbol, key) => {
    try {
        const res = await fetch(`https://restv2.fireant.vn/symbols/${symbol}/financial-indicators`, {
            headers: {
                Authorization: `Bearer ${key}`,
                Host: "restv2.fireant.vn"
            }
...
const getRatioCafeF = async (symbol) => {
    try {
        ...
        const Year = [], EPS = [], BV = [], PE = [], ROA = [], ROE = [], ROS = [], GOS = [], DAR = []
