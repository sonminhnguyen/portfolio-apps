...
import { calculateDiscountRate, calculateInitialIV, toRender } from '../vmiUS/function'
import { growthRate5Y, growthRate10Y } from "./functionVN";

const StockValuationVN = () => {
	const { profile, incomeStatement, balanceSheet, cashFlow, ratio, ratioCafeF } = useContext(vmiContext);
	...
	useEffect(() => {
		if (
			...
			const capitalExpenditure = cashFlow.find(element => element.id === 201).values.value.slice(-1)[0]
			const freeCashflow = operatingCashflow + capitalExpenditure
			const cashAndShortTermInvestments = balanceSheet.find(element => element.id === 1010102).values.value.slice(-1)[0]
				+ balanceSheet.find(element => element.id === 10102).values.value.slice(-1)[0]
