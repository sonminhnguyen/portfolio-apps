...
const getIncomeStatement = async (symbol) => {
    ...
    try {
        ...
        const results = await yahooFinance.quoteSummary('PLTR', { modules: [ "incomeStatementHistory" ] });
...
const getBalanceSheet = async (symbol) => {
    ...
    try {
        const results = await yahooFinance.quoteSummary(symbol, { modules: ["balanceSheetHistoryQuarterly"] });
...
const getCashFlow = async (symbol) => {
    ...
    try {
        const results = await yahooFinance.quoteSummary(symbol, { modules: ["cashflowStatementHistory"] });
...
const getRatio = async (symbol, key) => {
    try {
        ...
        const { defaultKeyStatistics } = await yahooFinance.quoteSummary(symbol, { modules: ["defaultKeyStatistics"] });
        const { financialData } = await yahooFinance.quoteSummary(symbol, { modules: ["financialData"] });
        const { summaryDetail } = await yahooFinance.quoteSummary(symbol, { modules: ["summaryDetail"] });
